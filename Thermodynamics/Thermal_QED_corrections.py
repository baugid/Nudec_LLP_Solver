...
def dP_2dz(x,z):
    ...
    Integrand_tmp2 = 2*y**2/z**2*np.exp((y**2 + x**2)**(1/2)/z)/(np.exp((y**2 + x**2)**(1/2)/z) +1)**2

    tmp1 = integrate.simpson(Integrand_tmp1,x=y)

    tmp2 = integrate.simpson(Integrand_tmp2,x=y)

    dP_2dz = -e**2*z/(6*np.pi**2)*tmp1 -e**2*z**2/(12*np.pi**2)*tmp2 - e**2/(4*np.pi**2)*tmp1*tmp2
...
def Thermal_QED_corrections_to_z(x,z): #(A.16) and (A.17)
    ...
    G2_1 = 2*np.pi*alpha*((1/w)*(K(x,z)/3 + 2*K(x,z)**2 - J(x,z)/6 - K(x,z)*J(x,z)) + (dKdw(x,z)/6 - K(x,z)*dKdw(x,z) + dJdw(x,z)/6 + dJdw(x,z)*K(x,z) + J(x,z)*dKdw(x,z)))


    G2_2 = -8*np.pi*alpha*(K(x,z)/6 + J(x,z)/6 - K(x,z)**2/2 + K(x,z)*J(x,z)) + 2*np.pi*alpha*w*(dKdw(x,z)/6 - K(x,z)*dKdw(x,z) + dJdw(x,z)/6 + dJdw(x,z)*K(x,z) + J(x,z)*dKdw(x,z))


    G3_1 = e**3/(4*np.pi)*(K(x,z) + w**2/2*k(x,z))**(1/2)*(1/w*(2*J(x,z) -4*K(x,z)) - 2*dJdw(x,z) - w**2*djdw(x,z) - w*(2*k(x,z) + j(x,z)) - (2*J(x,z) + w**2*j(x,z))*(w*(k(x,z)-j(x,z)) + dKdw(x,z))/(2*(2*K(x,z) + w**2*k(x,z))))


    G3_2 = e**3/(4*np.pi)*(K(x,z) + w**2/2*k(x,z))**(1/2)*((2*J(x,z) + w**2*j(x,z))**2/(2*(2*K(x,z) + w**2*k(x,z))) - 2/w*dYdw(x,z) - w*(3*dJdw(x,z) + w**2*djdw(x,z)))
...
def dKdw(x,z):
    ...
    Integrand_dKdw = -w*u**2*(1/(u**2 + w**2)**(3/2)*1/(np.exp((u**2+w**2)**(1/2)) + 1) + 1/(u**2 + w**2)*np.exp((u**2+w**2)**(1/2))/(np.exp((u**2+w**2)**(1/2))+1)**2)
...
def dJdw(x,z):
    ...
    Integrand_dJdw = w*u**2/(u**2 + w**2)**(1/2)*(np.exp((u**2+w**2)**(1/2))/(np.exp((u**2+w**2)**(1/2)) + 1)**2 - 2*np.exp(2*(u**2+w**2)**(1/2))/(np.exp((u**2+w**2)**(1/2)) + 1)**3)
...
def dYdw(x,z):
    ...
    Integrand_dYdw = w*u**4/(u**2 + w**2)**(1/2)*(np.exp((u**2+w**2)**(1/2))/(np.exp((u**2+w**2)**(1/2)) + 1)**2 - 2*np.exp(2*(u**2+w**2)**(1/2))/(np.exp((u**2+w**2)**(1/2)) + 1)**3)
...
def djdw(x,z):
    ...
    Integrand_djdw =  w/(u**2 + w**2)**(1/2)*(np.exp((u**2+w**2)**(1/2))/(np.exp((u**2+w**2)**(1/2)) + 1)**2 - 2*np.exp(2*(u**2+w**2)**(1/2))/(np.exp((u**2+w**2)**(1/2)) + 1)**3)
